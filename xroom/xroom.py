...
def load_people_track_model(gpu=False):
    return PeopleTrack(model=WEIGHT_YOLOV8SEG_PATH, gpu=gpu)
...
def load_flow_model(background_path, gpu=False, ocr_refresh_thres=800,
                    ...
                    multi_process=-1, input_scale=0.5):
...
control_path_video = pannel_control.text_input('🎞️ Video Path:',
                                                '../video/sample-1.mp4')
...
pannel_setting_more.subheader('Optical Char Recognition')
...
box_header_icon,_,_,_,_,_,_,_,_,_  = pannel_body.columns(10)
...
flow_model = load_flow_model(conf_background_path, conf_gpu_acceleration,
                             ...
                             conf_multi_process, conf_input_scale)
